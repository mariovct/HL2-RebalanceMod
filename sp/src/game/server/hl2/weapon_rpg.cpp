...
void CMissile::AccelerateThink( void )
{
	...
	SetAbsVelocity( vecForward * sk_rpg_missile_speed.GetFloat() );
...
// Purpose: 
//-----------------------------------------------------------------------------
#define	RPG_HOMING_SPEED	0.125f//bookmark
...
BEGIN_DATADESC( CWeaponRPG )
	...
	DEFINE_FIELD( m_bHasToggled,        FIELD_BOOLEAN ),
...
void CWeaponRPG::ItemPostFrame( void )
{
	...
	if ( ( m_bInitialStateUpdate ) && ( GetActivity() != ACT_VM_DRAW )  && !m_bHasToggled  )//bookmark
	{
		StartGuiding();
		m_bInitialStateUpdate = false;
	}
...
{
	//Can't have an active missile out
	if ( m_hMissile != NULL && IsGuiding() )//bookmark
		return false;
